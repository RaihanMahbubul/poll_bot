...
# ---!!! /start কমান্ড হ্যান্ডলার (বিস্তারিত নির্দেশনা সহ) !!!---
async def start_command(update: telegram.Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    instructions = """
👋 আসসালামু আলাইকুম! [SOT] পোল বট-এ আপনাকে স্বাগতম।

এই বটটি আপনার টেক্সট মেসেজকে সুন্দর কুইজ পোলে রূপান্তর করে আপনার চ্যানেলে পোস্ট করতে পারে।

**বট ব্যবহারের সম্পূর্ণ নিয়মাবলী:**

**ধাপ ১: টার্গেট চ্যানেল সেট করা (শুধু প্রথমবার)**
বটকে বলুন কোন চ্যানেলে পোস্ট করতে হবে।
* কমান্ড দিন: `/setchannel <channel_id_or_@username>`
* উদাহরণ (প্রাইভেট চ্যানেল): `/setchannel -100123456789`
* উদাহরণ (পাবলিক চ্যানেল): `/setchannel @MyQuizChannel`
*(বটকে অবশ্যই সেই চ্যানেলের অ্যাডমিন হতে হবে এবং পোল পোস্ট করার অনুমতি থাকতে হবে)*

**ধাপ ২: প্রশ্ন পাঠানো**
আপনার প্রশ্ন, অপশন, সঠিক উত্তর, ব্যাখ্যা এবং সাফিক্স (ট্যাগ) নিচের মতো সাজিয়ে বটকে টেক্সট মেসেজ করুন:

প্রশ্ন ১? [ট্যাগ-১] (ক) অপশন ১ (খ) অপশন ২ (গ) অপশন ৩ সঠিক উত্তর: (ক) ব্যাখ্যা: এটি হলো ব্যাখ্যা...

প্রশ্ন ২? [ট্যাগ-২] (ক) অপশন ১ (খ) অপশন ২ সঠিক উত্তর: (খ)

* **[ট্যাগ] (ঐচ্ছিক):** প্রতিটি প্রশ্নের শেষে `[ব্র্যাকেটের মধ্যে]` ট্যাগ দিলে, বট স্বয়ংক্রিয়ভাবে সেটিকে প্রশ্নের শেষে যোগ করবে (যেমন: `[MAT 23-24]`)।
* **ব্যাখ্যা (ঐচ্ছিক):** "ব্যাখ্যা:" লিখলে বট সেটি পোলে যুক্ত করবে।

**(দ্রষ্টব্য: বট স্বয়ংক্রিয়ভাবে প্রতিটি প্রশ্নের আগে `[SOT]` যোগ করে দেবে।)**

**ধাপ ৩: সূচনা বার্তা (Intro Message) পাঠানো**
...
**অন্যান্য কমান্ড:**
* `/cancel` - যেকোনো সময় কোনো কাজ (যেমন সূচনা বার্তার জন্য অপেক্ষা) বাতিল করতে এই কমান্ড দিন।
* `/help` - কমান্ডগুলোর একটি সংক্ষিপ্ত তালিকা দেখতে এই কমান্ড দিন।
"""
    
    await update.message.reply_text(
        instructions,
        parse_mode=ParseMode.MARKDOWN # <-- সুন্দর ফরম্যাটিং এর জন্য
    )
